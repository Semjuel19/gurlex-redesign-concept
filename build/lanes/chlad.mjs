/* ===========================================================================
   Lane 01 — Karpatský chlad / alpine cold
   Scene: a purchasing manager for a Tatras hotel group opens the catalogue at
   9am on a bright winter morning, snow-glare coming off the window, wanting to
   place a seasonal order quickly. That forces light, and it forces speed of
   scanning over atmosphere.
   Voice words: cold, mineral, exact.
   Reference objects: Nordic mountain wayfinding, a printed route guidebook.
   Colour: committed — snow and spruce carry it, one rescue-orange signal.
   Type: Familjen Grotesk alone, weight and size doing the work.
   The 20 brands are an index of routes, not a grid of cards.
   =========================================================================== */

import {
  site, nav, hero, facts, about, brands, brandsMeta, products, shelf, makers,
  downloads, contact,
} from '../content.mjs'
import {
  page, fontFace, fbIcon, chevIcon, arrowIcon, footerBlocks, disclaimerMarkup,
  bottleSrc, brandSrc,
} from '../partials.mjs'

const navItems = () =>
  nav
    .map((item) =>
      item.sub
        ? `<li class="nav__has-sub">
            <button type="button" class="nav__toggle" id="dl-toggle" aria-expanded="false" aria-controls="dl-menu">${item.label} ${chevIcon()}</button>
            <ul class="nav__sub" id="dl-menu">
              ${item.sub.map((s) => `<li><a href="${s.href}">${s.label}</a></li>`).join('\n              ')}
            </ul>
          </li>`
        : `<li><a href="${item.href}">${item.label}</a></li>`
    )
    .join('\n        ')

const localeRow = () =>
  `<ul class="locales" aria-label="Jazyk">
          <li><a href="https://www.gurlex.sk/" aria-current="true" hreflang="sk">SK</a></li>
          <li><a href="https://www.gurlex.cz/" hreflang="cs">CZ</a></li>
          <li><a href="https://www.gurlex.pl/" hreflang="pl">PL</a></li>
          <li><a href="https://www.gurlex.hu/" hreflang="hu">HU</a></li>
        </ul>`

const ft = footerBlocks()

export default function chlad() {
  const body = `
<header class="hdr" id="top">
  <div class="wrap hdr__in">
    <a class="hdr__mark" href="#top" aria-label="Gurlex, domovská stránka">
      <img src="../assets/brand/gurlex-wordmark-chlad.webp" alt="Gurlex" width="238" height="29">
    </a>

    <button class="burger" id="burger" type="button" aria-expanded="false" aria-controls="nav" aria-label="Menu">
      <span></span><span></span><span></span>
    </button>

    <nav class="nav" id="nav" aria-label="Hlavná navigácia">
      <ul class="nav__list">
        ${navItems()}
      </ul>
      <div class="hdr__aside">
        ${localeRow()}
        <a class="social" href="${site.facebook}" rel="noopener" aria-label="Facebook">${fbIcon(17)}</a>
      </div>
    </nav>
  </div>
</header>

<main id="main">

  <!-- ── hero ─────────────────────────────────────────────────────────── -->
  <section class="hero">
    <div class="wrap hero__in">
      <div class="hero__copy">
        <p class="kicker" data-reveal>
          <span class="kicker__tri" aria-hidden="true"></span>
          ${hero.kicker}
          <span class="kicker__sep" aria-hidden="true"></span>
          <span class="num">${hero.since}</span>
        </p>

        <h1 class="hero__h">
          <span class="line" data-reveal>${hero.lines[0]}</span>
          <span class="line line--mark" data-reveal>${hero.lines[1]}<i class="mark" aria-hidden="true"></i></span>
          <span class="line" data-reveal>${hero.lines[2]}</span>
        </h1>

        <p class="hero__lede" data-reveal>${hero.lede}</p>

        <div class="hero__cta" data-reveal>
          <a class="btn btn--primary" href="#znacky">${hero.ctaPrimary}</a>
          <a class="btn btn--quiet" href="${site.catalogPdf}">${hero.ctaSecondary}<span class="btn__tag">PDF</span></a>
        </div>
      </div>

      <figure class="hero__object" data-reveal>
        <div class="hero__hatch" aria-hidden="true"></div>
        <picture>
          <source srcset="../assets/bottle-hero.avif" type="image/avif">
          <img src="../assets/bottle-hero.webp" alt="${hero.bottleAlt}"
               width="480" height="751" fetchpriority="high" decoding="async">
        </picture>
      </figure>
    </div>

    <div class="wrap">
      <dl class="facts">
        ${facts
          .map(
            (f) => `<div class="fact" data-reveal>
          <dt>${f.k}</dt>
          <dd class="${f.num ? 'num' : ''}"${f.num ? ` data-count="${f.num}"` : ''}>${f.v}</dd>
        </div>`
          )
          .join('\n        ')}
      </dl>
    </div>
  </section>

  <!-- ── shelf ────────────────────────────────────────────────────────── -->
  <section class="shelf" aria-labelledby="shelf-h">
    <h2 class="sr" id="shelf-h">Vybrané z portfólia</h2>
    <ul class="wrap shelf__row">
      ${shelf
        .map(
          (b) =>
            `<li data-reveal><img src="${bottleSrc(b.file)}" alt="${b.alt}" width="400" height="800" loading="lazy" decoding="async"></li>`
        )
        .join('\n      ')}
    </ul>
  </section>

  <!-- ── about ────────────────────────────────────────────────────────── -->
  <section class="about" id="o-nas" aria-labelledby="about-h">
    <div class="wrap about__in">
      <div class="about__side">
        <p class="sec-n num">01</p>
        <h2 id="about-h" data-reveal>${about.heading}</h2>
      </div>
      <div class="about__body">
        <p class="lead" data-reveal>${about.paras[0]}</p>
        ${about.paras
          .slice(1)
          .map((p) => `<p data-reveal>${p}</p>`)
          .join('\n        ')}
      </div>
    </div>
  </section>

  <!-- ── brands: the index ────────────────────────────────────────────── -->
  <section class="idx" id="znacky" aria-labelledby="idx-h">
    <div class="wrap">
      <div class="idx__head">
        <div>
          <p class="sec-n num">02</p>
          <h2 id="idx-h" data-reveal>${brandsMeta.heading}</h2>
        </div>
        <p class="idx__note" data-reveal>${brandsMeta.note}</p>
      </div>

      <div class="idx__cols" aria-hidden="true">
        <span class="num">№</span><span>Značka</span><span>Vlastný web</span><span>Sortiment</span>
      </div>

      <ul class="idx__list" id="brand-list">
        ${brands
          .map(
            (b, i) => `<li class="row" data-reveal>
          <a href="https://www.gurlex.sk/produkty/${b.slug}">
            <span class="row__n num">${String(i + 1).padStart(2, '0')}</span>
            <span class="row__plate">
              <img src="${brandSrc(b.slug)}" alt="${b.alt || b.name}" width="240" height="150" loading="lazy" decoding="async">
            </span>
            <span class="row__name">${b.name}</span>
            <span class="row__ext">${b.ext ? b.ext : '<i aria-hidden="true">—</i>'}</span>
            <span class="row__n2 num">${products(b.n)}</span>
            <span class="row__go" aria-hidden="true">${arrowIcon()}</span>
          </a>
        </li>`
          )
          .join('\n        ')}
      </ul>

      <p class="idx__sum num" data-reveal>Spolu <strong>${brandsMeta.total}</strong> produktov v ${brands.length} značkách</p>
    </div>
  </section>

  <!-- ── makers ───────────────────────────────────────────────────────── -->
  <section class="mk" id="vyrobcovia" aria-labelledby="mk-h">
    <div class="wrap">
      <div class="mk__head">
        <p class="sec-n num">03</p>
        <h2 id="mk-h" data-reveal>${makers.heading}</h2>
      </div>
      <div class="mk__row">
        ${makers.items
          .map(
            (m) => `<a class="mk__card" href="${m.url}" rel="noopener" data-reveal>
          <span class="mk__logo"><img src="../assets/brand/${m.logo}.webp" alt="${m.alt}" width="300" height="120" loading="lazy" decoding="async"></span>
          <strong class="mk__name">${m.name}</strong>
          <span class="mk__desc">${m.desc}</span>
          <span class="mk__go">${m.host} ${arrowIcon()}</span>
        </a>`
          )
          .join('\n        ')}
      </div>
    </div>
  </section>

  <!-- ── downloads ────────────────────────────────────────────────────── -->
  <section class="dl" id="na-stiahnutie" aria-labelledby="dl-h">
    <div class="wrap dl__in">
      <div class="dl__side">
        <p class="sec-n num">04</p>
        <h2 id="dl-h" data-reveal>${downloads.heading}</h2>
      </div>
      <ul class="dl__list">
        ${downloads.items
          .map(
            (d) => `<li data-reveal>
          <a href="${d.url}">
            <span class="dl__name">${d.name}</span>
            <span class="dl__kind">${d.kind}</span>
            <span class="dl__go" aria-hidden="true">${arrowIcon()}</span>
          </a>
        </li>`
          )
          .join('\n        ')}
      </ul>
    </div>
  </section>

</main>

<!-- ── footer ─────────────────────────────────────────────────────────── -->
<footer class="ft" id="kontakt">
  <div class="wrap">
    <div class="ft__head">
      <p class="sec-n num">05</p>
      <h2>${contact.heading}</h2>
    </div>

    <div class="ft__grid">
      <div class="ft__col">
        <h3>Adresa</h3>
        ${ft.address}
      </div>
      <div class="ft__col">
        <h3>Telefón</h3>
        ${ft.phone}
        <h3 class="sp">Fax</h3>
        ${ft.fax}
      </div>
      <div class="ft__col">
        <h3>E-mail</h3>
        ${ft.email}
        <h3 class="sp">Sociálne siete</h3>
        ${ft.social}
      </div>
      <div class="ft__col">
        <h3>Krajiny</h3>
        ${ft.locales}
      </div>
    </div>

    <p class="ft__resp">${ft.responsibility}</p>

    <div class="ft__base">
      ${ft.base}
      ${ft.credit}
    </div>

    ${disclaimerMarkup()}
  </div>
</footer>`

  return page({
    lane: 'chlad',
    themeColor: '#f2f6f7',
    preload: ['familjen-grotesk-latin.woff2', 'familjen-grotesk-latin-ext.woff2'],
    fontCss: fontFace('Familjen', 'familjen-grotesk', { weight: '400 700' }),
    body,
  })
}
